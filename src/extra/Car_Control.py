...
def estimate_lanes(lines):
    # Count the occurrences of each line slope
    slope_counts = {}

    for line in lines:
        x1, y1, x2, y2 = line[0]

        # Skip lines with zero differences
        if x2 - x1 == 0:
            continue
    ...
    if len(slope_counts) > 0:
        # Select the line with the maximum occurrence
        dominant_slope = max(slope_counts, key=slope_counts.get)

        # Filter the lines that have the dominant slope
        dominant_lines = [line for line in lines if (line[0][3] - line[0][1]) / (line[0][2] - line[0][0]) == dominant_slope]

        # Merge and extrapolate the dominant lines
        merged_lines = merge_lines(dominant_lines)
    else:
        merged_lines = []
...
def merge_lines(lines):
    ...
    if len(lines) > 0:
        # Separate the line coordinates into x and y arrays
        x = np.array([line[0][0] for line in lines] + [line[0][2] for line in lines])
        y = np.array([line[0][1] for line in lines] + [line[0][3] for line in lines])

        # Fit a linear regression line to the coordinates
        regression = np.polyfit(y, x, deg=1)

        # Extrapolate the line to the top and bottom of the region of interest
        y_bottom = np.max(y)
        y_top = np.min(y)

        x_bottom = np.polyval(regression, y_bottom)
        x_top = np.polyval(regression, y_top)

        # Create a merged line using the extrapolated coordinates
        merged_line = [[int(x_bottom), int(y_bottom), int(x_top), int(y_top)]]
...
def detect_lanes(video_path):
    ...
    while cap.isOpened():
        ...
        cars = car_cascade.detectMultiScale(expanded_roi_gray_frame, 2.3, 3)
