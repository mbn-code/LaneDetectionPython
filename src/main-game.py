...
def calculate_dot_positions(lines):
    ...
    for line in lines:
        for x1, y1, x2, y2 in line:
            dot_positions.append((x1, y1))
            dot_positions.append((x2, y2))
...
def main():
    ...
    roi_vertices = np.array([[(int(x * SCREEN_WIDTH), int(y * SCREEN_HEIGHT)) for x, y in ROI_VERTICES_RATIO]], dtype=np.int32)
